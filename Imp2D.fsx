    ...
    let rec loop () =
        actor {
            ...
            | Activate1 (actorNum, sum, weight) ->
                ...
                //         flag <- false
                // if flag then
                //     let random = (System.Random()).Next(0, list1.Length)
                //     nodeMap.[list1.[random]] <!

                let mutable random =
                    (System.Random()).Next(0, list1.Length + 1)

                let mutable random1 = 0
                if random = list1.Length then
                    random1 <- (System.Random()).Next(1, numnodes + 1)
                else
                    while (random < list1.Length
                           && completedActors.Contains(list1.[random])) do
                        random <- (System.Random()).Next(0, list1.Length + 1)
                    if random = list1.Length
                    then random1 <- (System.Random()).Next(1, numnodes + 1)
                    else random1 <- list1.[random]
                // while completedActors.Contains(list1.[random]) do
                //     random <- (System.Random()).Next(0, list1.Length)
                // printfn
                //     "ACTOR:%d RANDOM:%d SUM1:%A SUM2: %A SUM3:%A DIFF1:%A DIFF2:%A"
                //     actorNum
                //     list1.[random]
                //     sum1
                //     sum2
                ...
                s <- s / 2.0
                w <- w / 2.0
                nodeMap.[random1] <! Activate1(random1, s, w)
            ...
            | Activate2 (actorNum, rumor) ->
                if stop = 0 then
                    ...
                    if count < maxmessage then
                        ...
                        if random = list1.Length then
                            random1 <- (System.Random()).Next(1, numnodes + 1)
                            nodeMap.[random1] <! Activate2(random1, 1)
                        else
                            nodeMap.[list1.[random]]
                            <! Activate2(list1.[random], 1)
                    //printfn "hello actor %d" random
                    // nodeMap.[list1.[random]] <! Activate(list1.[random], 1)
    ...
    let rec loop () =
        actor {
            ...
            | Activate (total, startNode, algo) ->
                if algo = "gossip" then
                    printfn
                        "Beginning Gossip Algorithm for Imperfect 2D Topology for %d nodes with start node as %d"
                        numnodes
                        startNode
                    ...
                    printfn
                        "Beginning PushSum Algorithm for Imperfect 2D Topology for %d nodes with start node as %d"
                        numnodes
                        startNode
                    neighMap <- neighMap.Add(1, [ 2; 1 + root ])
                    neighMap <- neighMap.Add(numnodes, [ numnodes - 1; numnodes - root ])
